  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{当前限制}
        \begin{itemize}
          \item 当前机器未检测到 \code{nvidia-smi}
          \item 未安装 \code{torch/transformers}
          \item 本次结果是本机可复现实验，不是 7B GPU 真模型结果
          \item 已提供 HuggingFace/TruthfulQA 真实模型入口
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{DoLa Decoding Pipeline}
  ...
  \begin{block}{本项目实现}
    ...
    GPU 脚本使用 HuggingFace 模型 hidden states 和 \code{lm\_head} 计算真实层 logits。
  \end{block}
\end{frame}
...
\begin{frame}{当前环境}
  ...
  \begin{alertblock}{说明}
    因环境限制，本 PPT 使用当前可复现实验结果；
    后续 GPU 验证将用 \code{scripts/run\_hf\_mc\_eval.py} 跑 TruthfulQA。
  \end{alertblock}
\end{frame}
...
\begin{frame}{结论}
  \begin{itemize}
    ...
    \item 当前本机实验中，DoLa 的 accuracy/truthfulness 从 0.706 提升到 0.882。
...
\begin{frame}{下一步提升方向}
  \begin{enumerate}
    \item GPU 上跑真实模型与 TruthfulQA，补真实日志和 GPU 使用截图。
    \item 与官方 DoLa 结果对齐：模型、split、layer 参数、relative top。
