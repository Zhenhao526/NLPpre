...
\subtitle{本机复现实验与 Colab 真实模型补充结果}
\author{自然语言处理课程大作业}
\institute{NLPpre}
\date{\today}
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{当前限制}
        \begin{itemize}
          ...
          \item 已完成 TruthfulQA-MC 全量 817 条
          \item LLaMA-7B 仍需 24GB+ GPU
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{当前环境}
  ...
  \begin{alertblock}{说明}
    本机实验用于解释机制；Colab 实验用于验证真实 HuggingFace 模型评测链路。
    LLaMA-7B 主结果仍需更大显存环境补跑。
  \end{alertblock}
\end{frame}
...
\begin{frame}{Colab 真实模型补充实验}
  ...
  \begin{alertblock}{解释}
    ...
    因此该结果证明真实 GPU 流程打通，不等价于论文 LLaMA 主结果复现。
  \end{alertblock}
\end{frame}
...
\begin{frame}{下一步提升方向}
  \begin{enumerate}
    \item 在 24GB+ GPU 上补 LLaMA-7B TruthfulQA-MC 和 FACTOR。
    \item 与官方 DoLa 结果对齐：模型、split、layer 参数、relative top、prompt。
