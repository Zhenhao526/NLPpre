...
\subtitle{本机机制实验、Colab 补充实验与官方 LLaMA-7B 复现结果}
\author{自然语言处理课程大作业}
\institute{NLPpre}
\date{\today}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{已完成}
        \begin{itemize}
          ...
          \item Colab Pythia-1.4B 全量 TruthfulQA-MC
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{结果边界}
        \begin{itemize}
          ...
          \item Pythia-1.4B 用于低显存真实模型补充验证
          \item 论文设置对齐以官方 DoLa LLaMA-7B 为主
          \item 当前主结果来自 TruthfulQA-MC 与 FACTOR 官方脚本
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{实验环境与证据层级}
  ...
  \begin{table}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.20\textwidth}
      ...
      Google Colab & T4 16GB，Pythia-1.4B & TruthfulQA-MC 817 条补充实验 \\
      服务器 & Ubuntu，2$\times$ RTX 3090 24GB，LLaMA-7B & 官方 TruthfulQA-MC 与 FACTOR 主复现 \\
      \bottomrule
    \end{tabular}
  \end{table}

  \begin{alertblock}{汇报原则}
    结论强度按证据层级区分：本机结果解释机制，Colab 结果说明低显存小模型表现，
    官方 DoLa LLaMA-7B 结果作为当前最接近论文设置的复现实验依据。
  \end{alertblock}
\end{frame}
...
\begin{frame}{Colab 真实模型补充实验}
  ...
  \begin{alertblock}{解释}
    ...
    因此该结果用于分析小模型与低显存条件下的稳定性，不作为论文设置主结果。
  \end{alertblock}
\end{frame}
